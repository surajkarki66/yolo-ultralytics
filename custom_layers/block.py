...
class C2(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, n: int = 1, shortcut: bool = True, g: int = 1, e: float = 0.5):
        ...
        # Channel selector convs to replace split (DPU compatible - no strided_slice)
        self.ch_select_a = nn.Conv2d(2 * self.c, self.c, 1, bias=False, groups=1)
        self.ch_select_b = nn.Conv2d(2 * self.c, self.c, 1, bias=False, groups=1)
        self._init_channel_selectors()

    def _init_channel_selectors(self):
        """Initialize channel selector weights to extract first/second half of channels."""
        with torch.no_grad():
            self.ch_select_a.weight.zero_()
            self.ch_select_b.weight.zero_()
            for i in range(self.c):
                self.ch_select_a.weight[i, i, 0, 0] = 1.0  # Select first c channels
                self.ch_select_b.weight[i, i + self.c, 0, 0] = 1.0  # Select second c channels

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through the CSP bottleneck with 2 convolutions."""
        out = self.cv1(x)
        a = self.ch_select_a(out)
        b = self.ch_select_b(out)
...
class C2f(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        out = self.cv1(x)
        y0 = self.ch_select_a(out)
        y1 = self.ch_select_b(out)
        # Build output by processing through bottlenecks without list indexing
        y_curr = y1
        y_outputs = [y0, y1]
        for m in self.m:
            y_curr = m(y_curr)
            y_outputs.append(y_curr)
        return self.cv2(torch.cat(y_outputs, 1))
...
class Bottleneck(nn.Module):
    ...
    def __init__(
        ...
        """
        super().__init__()
        # Compatibility for forks/callers that may pass (g, shortcut) positionally.
        if isinstance(g, bool) and isinstance(shortcut, (bool, int)):
            shortcut, g = bool(g), int(shortcut)
        c_ = int(c2 * e)  # hidden channels
...
class C2fAttn(nn.Module):
    ...
    def forward(self, x: torch.Tensor, guide: torch.Tensor) -> torch.Tensor:
        ...
        for m in self.m:
            y_curr = m(y_curr)
            y_outputs.append(y_curr)
        y_outputs.append(self.attn(y_curr, guide))
        return self.cv2(torch.cat(y_outputs, 1))
...
class RepNCSPELAN4(nn.Module):
    ...
    def __init__(self, c1: int, c2: int, c3: int, c4: int, n: int = 1):
        ...
        # Channel selector convs to replace split (DPU compatible - no strided_slice)
        self.ch_select_a = nn.Conv2d(c3, self.c, 1, bias=False, groups=1)
        self.ch_select_b = nn.Conv2d(c3, self.c, 1, bias=False, groups=1)
        self._init_channel_selectors()
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        y2 = self.cv2(y1)
        y3 = self.cv3(y2)
        return self.cv4(torch.cat([y0, y1, y2, y3], 1))
...
class ADown(nn.Module):
    ...
    def __init__(self, c1: int, c2: int):
        ...
        # Channel selector convs to replace split (DPU compatible - no strided_slice)
        c_in = c1  # Input channels after avg_pool2d
        c_half = c_in // 2
        self.ch_select_a = nn.Conv2d(c_in, c_half, 1, bias=False, groups=1)
        self.ch_select_b = nn.Conv2d(c_in, c_half, 1, bias=False, groups=1)
        self._init_channel_selectors(c_in, c_half)

    def _init_channel_selectors(self, c_in, c_half):
        """Initialize channel selector weights to extract first/second half of channels."""
        with torch.no_grad():
            self.ch_select_a.weight.zero_()
            self.ch_select_b.weight.zero_()
            for i in range(c_half):
                self.ch_select_a.weight[i, i, 0, 0] = 1.0  # Select first half channels
                self.ch_select_b.weight[i, i + c_half, 0, 0] = 1.0  # Select second half channels

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """Forward pass through ADown layer."""
        x = torch.nn.functional.avg_pool2d(x, 2, 1, 0, False, True)
        x1 = self.ch_select_a(x)
        x2 = self.ch_select_b(x)
        x1 = self.cv1(x1)
...
class C3k2(C2f):
    ...
    def __init__(
        ...
        self.m = nn.ModuleList(
            C3k(self.c, self.c, 2, shortcut=shortcut, g=g)
            if c3k
