"""Utilities for OBB quantized-model inference and evaluation (post_processing + evaluate only)."""
...
def non_max_suppression_obb(
    ...
    """OBB NMS: (b, N, 4+nc+1) = [xywh, cls..., angle]. Returns list of (k, 7) [xywh, conf, cls, angle]. Class-aware via center offset."""
    ...
    for xi, x in enumerate(pred):
        ...
        cls_id = x[:, 5:6]
        c = cls_id * float(max_wh)
        xywhr_nms = torch.cat((xywhr[:, :2] + c, xywhr[:, 2:4], xywhr[:, 4:5]), dim=-1)
        sorted_idx = torch.argsort(scores, descending=True)
        boxes_nms = xywhr_nms[sorted_idx]
        ious = batch_probiou(boxes_nms, boxes_nms).triu_(diagonal=1)
        pick = torch.nonzero(ious.max(dim=0)[0] < iou_threshold).squeeze(-1)
        keep = sorted_idx[pick][:max_det]
        out_list[xi] = x[keep]
    return out_list
