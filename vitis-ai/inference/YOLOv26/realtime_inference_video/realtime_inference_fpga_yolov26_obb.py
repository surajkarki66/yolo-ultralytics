...
def letterbox_transform_obb_boxes(xywha: np.ndarray, meta: dict) -> np.ndarray:
    ...
    # Remove padding and scale back for x, y, w, h (in-place)
    xywha[:, 0] = (xywha[:, 0] - pad_w) / gain  # x
    xywha[:, 1] = (xywha[:, 1] - pad_h) / gain  # y
    xywha[:, 2] /= gain  # width
    xywha[:, 3] /= gain  # height

    # Clip to original image boundaries
    xywha[:, 0] = np.clip(xywha[:, 0], 0, meta["w0"])
    xywha[:, 1] = np.clip(xywha[:, 1], 0, meta["h0"])
    xywha[:, 2] = np.clip(xywha[:, 2], 0, meta["w0"])
    xywha[:, 3] = np.clip(xywha[:, 3], 0, meta["h0"])
...
def decode_from_fpga(
    ...
    # angle decode (raw -> (sigmoid-0.25)*pi or already normalized)
...
def run_realtime_obb(
    ...
    for li, (h, w) in enumerate(level_hws):
        ...
        total_anchors += a
    anchors_cat = np.concatenate(anchors_parts, axis=0)
    ...
    while True:
        ...
        if input_is_nhwc:
            ...
            # Avoid creating an intermediate transposed array.
        ...
        runner.wait(jid)
        dpu_lat = (time.perf_counter() - dpu_start) * 1000.0         
        # ────────────────────────────────────────────────────────────────────
        ...
        end_to_end_lat = (time.perf_counter() - end_to_end_start) * 1000.0
        total_dpu_latency_ms += dpu_lat                                
        ...
        if frame_idx % 10 == 0:
            print(
                f"{frame_idx:<{col_w[0]}} | "
                f"{dpu_lat:<{col_w[1]}.2f} | "
                f"{1000.0/dpu_lat:<{col_w[2]}.2f} | "
                f"{end_to_end_lat:<{col_w[3]}.2f} | "
                f"{1000.0/end_to_end_lat:<{col_w[4]}.2f}"
            )
    ...
    avg_dpu = total_dpu_latency_ms / frame_idx                       
    avg_end_to_end = total_end_to_end_latency_ms / frame_idx
    print(f"AVERAGE DPU-ONLY:   {avg_dpu:.2f} ms  ({1000.0/avg_dpu:.2f} FPS)")
    print(f"AVERAGE END-TO-END: {avg_end_to_end:.2f} ms  ({1000.0/avg_end_to_end:.2f} FPS)")
    print(f"OVERHEAD (non-DPU): {avg_end_to_end - avg_dpu:.2f} ms per frame")             
    print(sep)


def parse_args():
    p = argparse.ArgumentParser(description="Realtime FPGA inference for YOLOv26 OBB using VART.")
