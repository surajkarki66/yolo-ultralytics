...
def letterbox_transform_boxes(xyxy: np.ndarray, meta: dict) -> np.ndarray:
    ...
    # Remove padding and scale back (in-place)
...
def run_realtime_detect(
    ...
    for s_expected in expected_strides:
        ...
        # Reuse precomputed feature map h,w for anchors.
    ...
    anchors_cat = np.concatenate(anchors_parts, axis=0)  # (total_anchors,2)
    stride_flat = np.concatenate(stride_parts, axis=0)   # (total_anchors,)
    scale4_cat = np.repeat(stride_flat[:, None], 4, axis=1)  # (total_anchors,4)
...
def parse_args():
    p = argparse.ArgumentParser(
        description="Realtime FPGA inference for YOLOv26 using VART."
    )
