...
def rotated_nms_class_aware(
    ...
    # class-aware offset on center like YOLOv26 main.py
    max_wh = 7680.0
...
def decode_from_fpga(
    ...
    # angle decode as in YOLOv26 main.py (raw -> (sigmoid-0.25)*pi or already normalized)
...
def run_realtime_obb(
    ...
    # Hardcoded YOLOv11 OBB xmodel output wiring:
    #   [0,1,2] = angle maps (1, H, W, 1) — strides 8, 16, 32
    #   [3,4,5] = box+cls maps (1, H, W, 4*reg_max+nc) — same spatial grid order
    # All feature maps are NHWC; decode transposes to NCHW.
    _YOLOV11_OBB_ANGLE_OUT = (0, 1, 2)
    _YOLOV11_OBB_BOX_OUT = (3, 4, 5)
    for i in _YOLOV11_OBB_ANGLE_OUT:
        s = out_shapes[i]
        if len(s) != 4 or int(s[-1]) != ne:
            raise ValueError(
                f"YOLOv11 OBB hardcode: expected output {i} NHWC with last dim {ne}, got shape {s}"
            )
    for i in _YOLOV11_OBB_BOX_OUT:
        s = out_shapes[i]
        if len(s) != 4 or int(s[-1]) != box_ch:
            raise ValueError(
                f"YOLOv11 OBB hardcode: expected output {i} NHWC with last dim {box_ch}, got shape {s}"
            )
    ...
    for i, s in enumerate(out_shapes):
        ...
        transpose_needed_flags.append(True)
        level_hw_per_out.append((int(s[1]), int(s[2])))

    box_indices = list(_YOLOV11_OBB_BOX_OUT)
    angle_indices = list(_YOLOV11_OBB_ANGLE_OUT)
    print(
        f"YOLOv11 OBB outputs (hardcoded): angle={angle_indices}, box+cls={box_indices} (all NHWC)"
    )
    ...
    while True:
        ...
        if input_is_nhwc:
            ...
            in_data[0][0, ...] = processed_img.transpose(2, 0, 1)
...
def parse_args():
    p = argparse.ArgumentParser(description="Realtime FPGA inference for YOLOv11 OBB using VART.")
